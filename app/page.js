...
const SalesRepBreakdown = ({ salespeople, salespeopleMonthly, salespeopleWeekly, repMargins, loading, currency, weeklyMonth, onWeeklyMonthChange, fyMonths = null, T, accent, exact = false }) => {
  ...
  // intermediate values, so column totals tie to the source to the cent.
            ...
            <tbody>
              {annualRows.length === 0 ? (
                ...
                <tr key={i} style={{ borderBottom: `1px solid ${T.borderFaint}` }}>
                  ...
                  <td style={{ ...cellStyle(r.revenue), color: accent, fontWeight: 700 }}>{money(r.revenue, currency)}</td>
        ...
        {activeStore.id === "luxe" && fyPayload(selectedYear) && (() => {
          ...
          return (
            <div style={{ marginBottom: 20, padding: "14px 18px", borderRadius: 12, background: `${accent}0c`, border: `1px solid ${accent}28`, fontSize: 12.5, color: T.textSub, lineHeight: 1.75 }}>
              ...
              {/* Ostendo feeds this dashboard from its nightly backup, so the
                  figures are always complete to the close of the last night that
